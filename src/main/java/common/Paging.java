...
public @Getter @Setter class Paging {
    ...
    String sType;					// 검색 타입
    String sUserId;					// 특정 사용자
    String sStartDate;				// 검색 시작날짜
    String sEndDate;				// 검색 종료날짜
    String sStartDate2;				// 검색 시작날짜
    String sEndDate2;				// 검색 종료날짜
    int sIntType;					// 숫자형 검색 타입
    String sStatus;					// 상태형 검색
    String broadId;					// 게시판 아이디
    String docStatus;
    ...
    public Paging(HttpServletRequest request) {
        ...
        int recordsPerPage = Integer.parseInt(myUtil.null2Zero(request.getParameter("size")));
        String sort_field = myUtil.null2Blank(request.getParameter("sorters[0][field]"));
        String sort_direct = myUtil.null2Blank(request.getParameter("sorters[0][dir]"));
        
        setSear(myUtil.null2Blank(request.getParameter("sear")).trim());
        setSear2(myUtil.null2Blank(request.getParameter("sear2")).trim());
        setSear3(myUtil.null2Blank(request.getParameter("sear3")).trim());
        setSear4(myUtil.null2Blank(request.getParameter("sear4")).trim());
        setSear5(myUtil.null2Blank(request.getParameter("sear5")).trim());
        setSear6(myUtil.null2Blank(request.getParameter("sear6")).trim());
        setSear7(myUtil.null2Blank(request.getParameter("sear7")).trim());
        setSear8(myUtil.null2Blank(request.getParameter("sear8")).trim());
        setSear9(myUtil.null2Blank(request.getParameter("sear9")).trim());
        setSType(myUtil.null2Blank(request.getParameter("sType")).trim());
        
        this.currentPageNo = Integer.parseInt(myUtil.null2Zero(request.getParameter("page")));
        this.sizeOfPage = 20;
